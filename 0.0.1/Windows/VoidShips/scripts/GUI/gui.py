...
class Hotbar(pg.sprite.Sprite):
	...
	def __init__(self, game):
		...
		self.rect.centerx = self.game.WIDTH / 2 + self.player.rect.width / 2
		self.rect.y = self.game.HEIGHT - self.rect.height - UI_MARGIN

		self.hblabel1 = (int(self.rect.x + self.rect.width / 3 - UI_MARGIN - 10), int(self.rect.centery + self.rect.height / 7))
		self.hblabel2 = (int(self.rect.x + (self.rect.width / 3) * 2 - UI_MARGIN - 13), int(self.rect.centery + self.rect.height / 7))
		self.hblabel3 = (int(self.rect.x + self.rect.width - UI_MARGIN - 16), int(self.rect.centery + self.rect.height / 7))
	...
	def update(self):
		...
		self.hblabel1 = (self.rect.x + self.rect.width / 3 - UI_MARGIN -
						 10, self.rect.centery + self.rect.height / 7)
		self.hblabel2 = (self.rect.x + (self.rect.width / 3) * 2 - UI_MARGIN -
						 13, self.rect.centery + self.rect.height / 7)
		self.hblabel3 = (self.rect.x + self.rect.width - UI_MARGIN -
						 16, self.rect.centery + self.rect.height / 7)

		# Spawn placeholder on the hotbar
		if self.player.fullinv[0]["item"] and not self.player.hotbar_display[0]:
			self.player.hotbar_display[0] = ItemPlaceholder(
				self.game, "hb1", self.player.fullinv[0]["item"])
		if self.player.fullinv[1]["item"] and not self.player.hotbar_display[1]:
			self.player.hotbar_display[1] = ItemPlaceholder(
				self.game, "hb2", self.player.fullinv[1]["item"])
		if self.player.fullinv[2]["item"] and not self.player.hotbar_display[2]:
			self.player.hotbar_display[2] = ItemPlaceholder(
				self.game, "hb3", self.player.fullinv[2]["item"])
		...
		if self.player.selected_slot != -1 and self.player.fullinv[self.player.selected_slot]["count"] > 0:
			...
			if self.game.player.vel.x > 0:
				self.player.holding.image = pg.transform.flip(
					ITEMS[self.player.fullinv[self.player.selected_slot]["item"]], true, false)
			if self.game.player.vel.x < 0:
...
class ItemPlaceholder(pg.sprite.Sprite):
	...
	def __init__(self, game, slot, item):
		...
		self.hand = self.game.WIDTH / 2 + self.game.player.rect.width / 2 + self.handoffset.x, self.game.HEIGHT / \
			2 + self.game.player.rect.height / 2 + self.handoffset.y
		...
		if self.slot in self.slots:
			...
			self.image = pg.transform.scale(
				self.oimage, (self.orect.width * 2, self.orect.height * 2))
			self.rect = self.image.get_rect()
	...
	def update(self):
		...
		for slot in self.slots:
			for ph in self.game.placeholders:
				if ph.slot == slot:
					ph.image = pg.transform.scale(
						ITEMS[self.player.fullinv[slot]["item"]], (self.rect.width * 2, self.rect.height * 2))
		...
		for slot in self.inv.empty_slots:
			...
			if self.slot in self.slots and self.inv.empty_slots[slot].collidepoint(mousepos) and self.player.on_inv:
				...
				if self.game.event.type == pg.MOUSEBUTTONDOWN:
...
class Inventory(pg.sprite.Sprite):
	...
	def __init__(self, game):
		...
		self.rect.center = vec(self.game.WIDTH / 2 + self.game.player.rect.width / 2, self.game.HEIGHT / 2 - self.offset)
		self.player = game.player

		self.font = pg.font.Font("textures/fonts/ttp.otf", 15)

		self.hotbar_img = pg.transform.scale(self.game.hotbar.image.copy(
		), (self.game.hotbar.rect.width * 2, self.game.hotbar.rect.height * 2))
		self.hotbar_rect = self.hotbar_img.get_rect()
		self.hotbar_rect.center = vec(self.game.WIDTH / 2 + self.game.player.rect.width / 2, self.game.HEIGHT / 2 + self.offset * 1.5)
	...
	def update(self):
		...
		if self.player.fullinv[0]["item"] and not self.player.inv_display[0]:
			self.player.inv_display[0] = ItemPlaceholder(
				self.game, 0, self.player.fullinv[0]["item"])
		if self.player.fullinv[1]["item"] and not self.player.inv_display[1]:
			self.player.inv_display[1] = ItemPlaceholder(
				self.game, 1, self.player.fullinv[1]["item"])
		if self.player.fullinv[2]["item"] and not self.player.inv_display[2]:
			self.player.inv_display[2] = ItemPlaceholder(
				self.game, 2, self.player.fullinv[2]["item"])
		if self.player.fullinv[3]["item"] and not self.player.inv_display[3]:
			self.player.inv_display[3] = ItemPlaceholder(
				self.game, 3, self.player.fullinv[3]["item"])
		if self.player.fullinv[4]["item"] and not self.player.inv_display[4]:
			self.player.inv_display[4] = ItemPlaceholder(
				self.game, 4, self.player.fullinv[4]["item"])
		if self.player.fullinv[5]["item"] and not self.player.inv_display[5]:
			self.player.inv_display[5] = ItemPlaceholder(
				self.game, 5, self.player.fullinv[5]["item"])
		if self.player.fullinv[6]["item"] and not self.player.inv_display[6]:
			self.player.inv_display[6] = ItemPlaceholder(
				self.game, 6, self.player.fullinv[6]["item"])
		if self.player.fullinv[7]["item"] and not self.player.inv_display[7]:
			self.player.inv_display[7] = ItemPlaceholder(
				self.game, 7, self.player.fullinv[7]["item"])
		if self.player.fullinv[8]["item"] and not self.player.inv_display[8]:
			self.player.inv_display[8] = ItemPlaceholder(
				self.game, 8, self.player.fullinv[8]["item"])
